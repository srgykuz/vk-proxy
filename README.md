...
Рекомендуется использовать Debian или Ubuntu.
...
Это запустит программу в постоянном режиме. После перезагрузки компьютера она будет запущена автоматически.
...
На Windows установка [ZBar](https://github.com/mchehab/zbar) затруднительна. Рекомендуется использовать [WSL](https://learn.microsoft.com/windows/wsl/install) и запускать [Linux-версию](#linux).
...
Выполните [это](#5-заполните-конфиг).
...
Адаптируйте инструкцию для [Linux](#linux) или [Windows](#windows) самостоятельно.
